...
└─ for iteration 1..N:
   ...
   ├─ Execute action (patch / tool_call / complete)
   ├─ Verify gate (python tests/test.py --skip-web --quiet) → VERIFY_RESULT
   │   └─ If FAIL:
...
## Running Tests
```bash
# Full webui smoke tests (35+ tests)
...
| LLM calls fail with 401 | Set `ANTHROPIC_API_KEY` / `GOOGLE_API_KEY` / `OPENAI_API_KEY` env var |
